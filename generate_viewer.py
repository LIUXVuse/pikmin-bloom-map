...
function copyJKOCode() {
  navigator.clipboard.writeText('396').then(() => {
    const el = document.getElementById('jko-code-copied');
    el.style.display = 'inline';
    setTimeout(() => { el.style.display = 'none'; }, 2000);
  });
}
function copyJKOAccount() {
  navigator.clipboard.writeText('901238319').then(() => {
    const el = document.getElementById('jko-account-copied');
    el.style.display = 'inline';
    setTimeout(() => { el.style.display = 'none'; }, 2000);
  });
}
...
<div id="bridge-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9998;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:16px;padding:28px 24px;max-width:360px;width:90%;text-align:center;position:relative;">
    ...
    <div style="background:#f9fafb;border-radius:10px;padding:14px;text-align:left;font-size:0.82rem;color:#374151;margin-bottom:12px;line-height:1.8">
      <b>使用條件：</b><br>
      📱 iPhone（iOS 16+）<br>
      💻 Windows 電腦<br>
      🎵 已安裝 iTunes<br>
    ...
    <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:12px;text-align:left;font-size:0.8rem;color:#9a3412;margin-bottom:12px;line-height:1.8">
      <b>⚠️ 使用說明：</b><br>
      1. 下載後，<b>右鍵 → 以系統管理員執行</b><br>
      2. 點「開始橋接」，等出現綠燈<br>
      3. 回到地圖，點任一景點的「帶我飛！」<br>
      <br>
      <span style="color:#b45309">本工具為個人製作，使用上若有問題請自行排除，不接受個別詢問 🙏</span>
    </div>
